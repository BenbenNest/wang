    ...
    public void onPreviewFrame(final byte[] data, Camera camera) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (in_bound >= setBound && clear >= setClear) {
                    isSuccess = true;
...
//                    doOCR(path);
                    ...
                    if (!idCardQuality.isfaculaePass) {
                        errorStr = "有光斑";
                    } else if (idCardQuality.isfaculaePass) {//&& idCardQuality.isShadowPass
                        Bitmap bitmap = ConUtil.cutImage(mIndicatorView.getPosition(), data, mICamera.mCamera,
                                mIsVertical);
                        String path = ConUtil.saveBitmap(BankCardScanActivity.this, bitmap);
//                        enterToResult(path, idCardQuality, clear, is_idcard, in_bound);
    ...
    private void enterToResult(String bank, String number) {
...
//        startActivity(intent);
    ...
    public void showBar(final boolean flag) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (flag) {
    ...
    public void doOCR(final String path) {
        showBar(true);
        try {
            String url = "https://api-cn.faceplusplus.com/cardpp/v1/ocrbankcard";
            RequestParams rParams = new RequestParams();
            Log.w("ceshi", "Util.API_OCRKEY===" + KeyUtil.API_SECRET + ", KeyUtil.API_OCRSECRET===" + KeyUtil.API_SECRET);
            ...
            asyncHttpclient.post(url, rParams, new AsyncHttpResponseHandler() {
                ...
                public void onSuccess(int statusCode, Header[] headers, byte[] responseByte) {
                    ...
                    try {
                        ...
                        String info = "";
                        JSONObject jObject = new JSONObject(successStr).getJSONArray("bank_cards").getJSONObject(0);
                        String bank = jObject.optString("bank");
                        String number = jObject.optString("number");
                        enterToResult(bank, number);
